...
`onyx-trace-flame-probe` is centered on observability work in Scala. The technical target is: Package a Scala local lab for flame analysis with deny and allow fixtures, explainable decision traces, and documented operating limits.
...
The score is deterministic. The current threshold is 156. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 5.
